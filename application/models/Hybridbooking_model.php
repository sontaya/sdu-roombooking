...
class Hybridbooking_model extends CI_Model {
    ...
    function save($data) {
       $result = $this->db->insert('hb_booking_info', $data);
    ...
    function update($id, $data) {
        $this->db->where('id', $id);
        $result = $this->db->update('hb_booking_info', $data);
        if ($result) {
            return true;
        } else {
            return false;
    ...
    function delete($id) {
        $this->db->where('id', $id);
        $result = $this->db->delete('hb_booking_info');
        if ($result) {
            return true;
        } else {
            return false;
	  	...
	  	$this->db->select('b.*, rm.room_tag, rm.name as room_name, rm.shortname as room_shortname , u.name, u.surname, u.name_faculty, u.name_department, u.line_sub, u.line_iat, u.line_exp,
	  			(
					case
						...
						when b.usage_category = "2" then "จัดอบรม"
						else ""
					end
				...
				', false);
		$this->db->from('hb_booking_info b');
		$this->db->join('hb_room_master rm', 'b.room_id = rm.id');

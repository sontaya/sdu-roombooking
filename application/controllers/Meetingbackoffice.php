...
class Meetingbackoffice extends MY_Controller
{
	...
	function form_admin($id = null){
		$data['title'] = "จองโดยเจ้าหน้าที่ (Meeting Admin)";

		$data['cssSrc'] = array();

		$data['jsSrc'] = array(
			'assets/js/meeting/mt-backoffice-form-admin.js',
		...
		$data['room_info'] = $this->Mtroom_model->list(array('conditions'=> $room_master_conditions));

		$this->data = $data;
		$this->content = 'meetingbackoffice/form_admin';
		$this->render_mt();
	...
	public function form_admin_store(){
		...
		if($this->input->post('form_mode') == "update"){
			...
			$this->Mtbooking_model->log($this->input->post('form_id'));
	...
	function booking_manage(){
		//
		$data['title'] = "รายการจองห้อง (Meeting Admin)";

		$data['jsSrc'] = array(
			'assets/js/meeting/mt-backoffice-init.js',
			'assets/js/meeting/mt-backoffice-booking-manage.js'
		);
		...
		$data['booking_lists'] = $this->Mtbooking_model->list(array('conditions'=> $criterias));
		$this->data = $data;
		$this->content = 'meetingbackoffice/manage_booking';
		$this->render_mt();
	...
	function booking_calendar($room_id = null){
		...
		if($room_id == null){
			$room_id = $this->global_data['default_mt_room'];
		}

		$data['title'] = "มุมมองปฏิทิน (Meeting Admin)";
		$data['cssSrc'] = array(
		...
		$data['jsSrc'] = array(
			...
			'assets/js/meeting/mt-backoffice-init.js',
			'assets/js/meeting/mt-backoffice-booking-calendar.js'
		);
		...
		//--Begin:: เงื่อนไขในการเลือกห้อง
			...
			$data['room_info1'] = $this->Mtroom_model->list(array('conditions'=> $room_master_conditions1));
			...
			$data['room_info2'] = $this->Mtroom_model->list(array('conditions'=> $room_master_conditions2));
		...
		$this->content = 'meetingbackoffice/manage_booking_calendar';
		$this->render_mt();
	...
	function get_booking_request(){
		...
		if($_POST){
			...
			$lists = $this->Mtbooking_model->list(array('conditions'=>  $_POST["query"]));
			...
			$lists = $this->Mtbooking_model->list();
			$data['meta'] = array(
				'page' => 1,
				'pages' => 1,
				'perpage' => -1,
	...
	function booking_approve(){
		...
		$res = $this->Mtbooking_model->update($target_id, $data);
	...
	function booking_delete(){
		...
		$res = $this->Mtbooking_model->delete($target_id);
	...
	function get_booking_info($id){
		...
		$res = $this->Mtbooking_model->list(array('conditions'=>$conditions));
	...
	function booking_cancel(){
		...
		//-- Log
		$this->Mtbooking_model->log($target_id);

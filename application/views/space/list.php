										...
										<tbody>
											...
											?>
												<tr>
													<th scope="row">
														<?php
															$staff_ids = array_map('trim', explode(',', $booking["staff_ids"]));
															$user_hrcode = trim($this->session->userdata('auth')['hrcode']);

															// More flexible matching
															$match_found = false;
															foreach ($staff_ids as $id) {
																if (trim($id) === $user_hrcode) {
																	$match_found = true;
																	break;
																}
															}
														...
														?>
																<div class="symbol symbol-40 symbol-light-success flex-shrink-0">
																	<span class="symbol-label font-size-h5 font-weight-bold">
																		<i class="icon-2x text-success flaticon2-user"></i>
																	</span>
																</div>
														<?php }else{ ?>
